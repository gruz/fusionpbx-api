	...
	if (!function_exists('th_order_by')) {
		...
		function th_order_by($field_name, $column_title, $order_by, $order, $app_uuid = '', $css = '', $http_get_params = '', $description = '') {
			...
			if (is_uuid($app_uuid) > 0) { 
				$app_uuid = urlencode("&app_uuid=".$app_uuid);
			...
			if ($order == "asc") {
				...
				$html .= "<a href='?order_by=".urlencode($field_name)."&order=desc".$app_uuid.$sanitized_parameters."' title='".urlencode($description)."'>".urlencode($column_title)."</a>";
			}
			else {
				$description .= 'sort(descending)';
				$html .= "<a href='?order_by=".urlencode($field_name)."&order=asc".$app_uuid.$sanitized_parameters."' title='".urlencode($description)."'>".urlencode($column_title)."</a>";
			}
			$html .= "</th>";
			return $html;
		}
