...
class RatchetServer extends RatchetServerBase
{
    ...
    public function onMessage(ConnectionInterface $conn, $input)
    {
        ...
        {
          throw new \Infrastructure\Exceptions\Socket\InvalidJSONInput();
          return;
        }
        ...
        if ($this->isThrottled($conn, 'onMessage')) {
            ...
            throw new \Infrastructure\Exceptions\Socket\TooManyMessages();
            $this->throttled = true;
        ...
        {
          throw new \Infrastructure\Exceptions\Socket\NoCommadException();
          return;
        }

        if (!$this->auth($conn) && !in_array($input->command,  ['post.login', 'post.login2'])) {
            throw new \Infrastructure\Exceptions\Socket\NeedToLoginFirst();
            return;
        }
        ...
        switch ($command_stripped[0]) {
          case 'post.login2':
            ...
            {
              throw new \Infrastructure\Auth\Exceptions\InvalidCredentialsException();
              return;
            }
...
return;
    ...
    private function throttle()
    {
        if ($this->isThrottled($this->conn, 'onOpen')) {
            ...
            $this->onError($this->conn, new \Infrastructure\Exceptions\Socket\TooManyConnectionAttempts());
            $this->throttled = true;
            $this->conn->close();
        }
    ...
    private function limit()
    {
        if ($connectionLimit = config('ratchet.connectionLimit') && $this->connections - 1 >= $connectionLimit) {
            ...
            $this->onError($this->conn, new \Infrastructure\Exceptions\Socket\TooManyConnections());
            $this->conn->close();
        }
